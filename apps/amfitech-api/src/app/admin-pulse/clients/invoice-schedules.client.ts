...
@Injectable()
export class InvoiceSchedulesClient {
  constructor(
    private readonly http: AdminPulseHttpService,
    @Inject(CACHE_MANAGER) private readonly cache: Cache,
  ) {}

  async listAll(token: string): Promise<InvoicesScheduleDto[]> {
    const key = `invoice-schedules:${token}`;
    const cached = await this.cache.get<InvoicesScheduleDto[]>(key);
    if (cached) return cached;
    const data = await fetchAllPages((page, pageSize) =>
      this.http.get<PageDto<InvoicesScheduleDto>>(
        '/relations/invoiceschedules',
        token,
        { page, pageSize },
      ),
    );
